...
class DeliveryController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return onlyShowBtn('Approved Payment Customer', route('admin.delivery.show', $data['id']));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
    ...
    public function delivery_product_index(Request $request)
    {
        if ($request->ajax()) {
            $data = Delivery::join('transaction as trx', 'delivery.transaction_id', '=', 'trx.id')
                ...
                ->orderBy('delivery.created_at', 'DESC')->get();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($data) {
