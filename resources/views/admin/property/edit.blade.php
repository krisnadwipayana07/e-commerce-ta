...
@section('page-back-button')
    <a href="{{ route('admin.property.index') }}" class="my-2 btn btn-secondary btn-sm"><i
                ...
                <div class="row mb-3">
                    <div class="col-md">
                        <div class="form-group">
                            ...
                            <select name="sub_category_property_id"
                                class="select-picker form-control @error('sub_category_property_id') is-invalid @enderror"
                                style="width:100%;">
                                    ...
                                    <optgroup label="{{ $category->name }}">
                                        ...
                                        @foreach ($category->sub_category_property as $sub_category)
                                            <option value="{{ $sub_category->id }}"
                                                {{ old('sub_category_property_id', $data->sub_category_property_id) == $sub_category->id ? 'selected' : '' }}>
                                                {{ $sub_category->name }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror">
                                ...
                                @foreach (returnStatus() as $key => $value)
                                    <option value="{{ $key }}"
                                        {{ old('status', $data->status) == $key ? 'selected' : '' }}>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                            @error('status')
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-12 my-2">
                        <img id="preview-myimg"
                            src="{{ $data->img ? url('/upload/admin/property/', $data->img) : 'https://cdn.pixabay.com/photo/2015/12/22/04/00/photo-1103595_960_720.png' }}"
