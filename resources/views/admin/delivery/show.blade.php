...
<div class="border-bottom py-3">
    <h6 class="font-weight-bold">Nomor Telepon</h6>{{ $data->account_number }}
</div>
<div class="border-bottom py-3">
...
<div class="my-5">
    ...
    <img id="preview-product_evidence" src="{{ $evidence ? url('/upload/admin/delivery/product/', $evidence->product_evidence) : 'https://cdn.pixabay.com/photo/2015/12/22/04/00/photo-1103595_960_720.png' }}" alt="preview image" style="height: 50vh;">

</div>
<div class="my-5">
    <p>Foto TTD Penerimaan Produk</p>
    <img id="preview-signature_evidence" src="{{ $evidence ? url('/upload/admin/delivery/signature/', $evidence->signature_evidence) : 'https://cdn.pixabay.com/photo/2015/12/22/04/00/photo-1103595_960_720.png' }}" alt="preview image" style="height: 50vh;">
</div>
<div class="py-3">
    ...
    <div class="form-group">
        <form action="{{ route('admin.delivery.change_status', $data->id) }}" method="POST">
            ...
            <div class="pb-1 d-flex gap-2">
                <select aria-label="Default select example" name="status" required id="payment_status" onchange="val()" class="form-select w-50">
    ...
    <form action="{{ route('admin.delivery.send_notifications') }}" method="POST">
        <div class="my-3 d-flex gap-2 message_confirmation">
            ...
            <input type="text" name="message" placeholder="Informasi Pertanyaan Hari pengiriman" class="form-control">
            <button type="submit" class="btn btn-primary">Kirim Pesan Notifikasi</button>
        </div>
    </form>
    @foreach ($notifications as $idx => $notif)
    @if ($idx === 0)
    <h5 class="text-center">Balasan</h5>
    @endif
    <div class="my-2">
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="customerReply" class="form-label">Pelanggan</label>
